let bootSequence = `PMAP: PCID enabled
...
function showLine(n) {
    // Mostramos la línea n de la secuencia de boot
    let line = bootSequence[n];

    // Creamos un nuevo <p> con la linea actual de la secuencia de boot
    let p = document.createElement('p');
    p.textContent = line;
    
    // Agregamos el nuevo <p> al div de display
    document.getElementById('display').appendChild(p);

    // Scrolleamos automáticamente hasta el fondo
    p.scrollIntoView();
}

function loaded(n) {
    // Esperamos un momento antes de hacer aparecer la siguiente línea
    setTimeout(function() {
        // Mostramos la línea actual
        showLine(n);

        // Mostramos la siguiente línea
        if (n+1 < bootSequence.length)
            loaded(n+1);
    }, 80 - (Math.random() % 20)); // 80 ms - (random entre 0 y 20)
}
